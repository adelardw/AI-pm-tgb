...
self_evaluator_pmt = """
Ты - критик ответ языковой модели. Твоя задача - переформулировать ответы модели
исходя от текущего контекста, ответа модели и запроса пользователя.
Подсказать в чем есть ошибка, что нужно исправить.
Ndj

"""
query_pmt = ("Ты эксперт по поиску информации. Твоя задача — сформулировать поисковые запросы на основе последнего сообщения пользователя и локального контекста.")

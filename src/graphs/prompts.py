...
image_dsc_pmt = "Ты - часть модуля память. И ты делаешь саммари диалога по запросу пользователя и приложенным изображениям."\
    "Задача стоит такая: дать описание каждого изображения учитывая контекст - просьбу пользователя. Сделай это так, чтобы "\
    "в будущем,  если пользователь попросил что - то вспомнить - ты из этого описания смог понять контекст, суть и ответить пользователю" \
    "Сделай кратко и понятно"
...
finalizer = "Необходимо выполнить следующее с текстом:"\
            "1. Заменить LaTeX формулы, если они есть, так, чтобы их было удобно читать постах в мессенджере телеграм, так и в целом"\
                "человеку, который не знаком с LaTeX документацией "\
            "2. Просто измени текст, не давай лишних комментариев от СЕБЯ"\
            "[КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО]"\
            "1. ЗАПРЕЩЕНО! Давать комментарии от себя, по типу: "\
            "Конечно, вот изменённый текст — формулы заменены на понятные обозначения, "\
            "всё адаптировано для удобного чтения в мессенджере, без лишних комментариев "
...
img_desc_prompt = RunnableLambda(lambda x: image_text_prompt(image_dsc_pmt,  x))
